...
X_train_cv, X_test_cv, X_train_cv_df, count_vect = tokenize_text(mydata_train_df, mydata_test_df)
...
# Evaluation of the performance on the test set using SVM Classifier
print ("\n\n KNN \n ----------------------- \n ")
clf3 = KNNTestingData(xtrain_tfidf, dataset_20_train, mydata_test_df, X_test_cv)

# using pipeline  test data
KNNTestingPipeline(mydata_train_df, mydata_test_df, dataset_20_test)

#test customized : 	
KNNTest(count_vect, tfidf_transformer, clf3, dataset_20_train)
...
# Evaluation of the performance on the test set using SVM Classifier
print ("\n\n Logistic regression \n ----------------------- \n ")
clf3 = Logistic_regTestingData(xtrain_tfidf, dataset_20_train, mydata_test_df, X_test_cv)

# using pipeline  test data
Logistic_regTestingPipeline(mydata_train_df, mydata_test_df, dataset_20_test)

#test customized : 	
Logistic_regTest(count_vect, tfidf_transformer, clf3, dataset_20_train)


# -----------------------------------------------------------------------------
print (colored("\n -------------------------  \n time duration compare :"))
for d in data_duration_compare:
